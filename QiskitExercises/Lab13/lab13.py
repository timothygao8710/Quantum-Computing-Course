...
def exercise_2(circuit, register, target):
    ...
    ancilla = QuantumRegister(len(register)-1, "taffoli")     # The name here isn't required, just for documentation
    ...
    for i in range(1, len(ancilla)):
        circuit.ccx(ancilla[i-1], register[i+1], ancilla[i])
    
    circuit.cz(ancilla[len(ancilla)-1], target)

    for i in range(len(ancilla)-1, 0, -1):
        circuit.ccx(ancilla[i-1], register[i+1], ancilla[i])
    circuit.ccx(register[0], register[1], ancilla[0])
...
def exercise_3(circuit, 
    ...
    # exercise_1(circuit, classical_bits=original_message, register=candidate_encryption_key)
    # exercise_1(circuit, classical_bits=encrypted_message, register=candidate_encryption_key)
    ...
    #Given that we know key is XOR
    #original[i] XOR candidate[i] = encrypted[i]
    #apply XOR original[i] to both sides
    # original[i] XOR encrypted[i] = candidate[i]

    for i in range(len(original_message)):
        if xor(original_message[i], encrypted_message[i]):
            circuit.x(candidate_encryption_key[i])
...
def exercise_5(oracle, number_of_qubits):
    ...
    iterations = round(pow(2, number_of_qubits / 2))
    ...
    #starting off with an all H
    circuit.h(qubits)
    for i in range(iterations):
        #All H is applied twice per iteration
        exercise_4(circuit, oracle, qubits, ancilla)

    circuit.measure(qubits, measurement)

    simulator = Aer.get_backend('aer_simulator')
    simulation = execute(circuit, simulator, shots=10)
    ...
    #Isn't it possible for there to be a possibility for other keys cuz its probabalistic
