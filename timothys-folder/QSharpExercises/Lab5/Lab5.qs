...
namespace Lab5 {
    ...
    operation Exercise1 (input : Qubit[], output : Qubit) : Unit {
        ...
        ///Aren't we suppose to flip the input register not the output qubit?
        for i in input{
            Controlled Z([i], output);
        }
    }
    ...
    operation Exercise3 (
        ...
        for i in input{
            ///use i == input[0]?
            // set res = res and (M(i) == M(input[0]));
            set res = res and (M(i) == Zero);
        }
    ...
    ///Why do you need to reset the output in the end
}
